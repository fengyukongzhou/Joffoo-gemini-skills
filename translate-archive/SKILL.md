...
description: Translates articles and documents between languages with three modes - quick (direct), normal (analyze then translate), and refined (analyze, translate, review, polish). Integrated with translate-archive for web clipping (using defuddle-cli) and automatic archiving to 03-Resources/. Use when user asks to "translate", "翻译", "精翻", "translate article", "translate to Chinese/English", "改成中文", "改成英文", "convert to Chinese", "localize", "本地化", or needs any document translation. Also triggers for "refined translation", "精细翻译", "proofread translation", "校对翻译", "快翻", "这篇文章翻译一下", or when a URL or file is provided with translation intent.
---
...
> Translation saved. To further review and polish, reply "精细润色" or "refine".
...
Custom audience descriptions are also accepted, e.g., `--audience "AI领域的初学者".`
...
Materialize source:
- **If URL**: Use `defuddle parse <url> --md` via `run_shell_command` to get clean markdown. If `defuddle` fails or is missing, fall back to `web_fetch`. Save result to `translate/{slug}.md`.
- **If File/Text**: Use as-is (inline text saves to `translate/{slug}.md`).

Then create output directory: `{source-dir}/{source-basename}-{target-lang}/`. Detect source language if `--from` not specified.
...
| `polish.md` | Refined | Final publication-quality translation |
...
- **Meaning over words**: Translate what the author means, not just what the words say. When a literal translation sounds unconventional or fails to convey the intended effect, restructure freely to express the same meaning in idiomatic target language
...
- **Translator's notes**: For terms, concepts, or cultural references that target readers may not understand -> due to jargon, cultural gaps, or domain-specific knowledge -> add a concise explanatory note in parentheses immediately after the term. The note should explain *what it means* in plain language, not just provide the English original. Format: `中文解释 (English original，简要背景说明).` Calibrate annotation depth to the target audience: general readers need more notes than technical readers. Only add notes where genuinely needed; do not over-annotate obvious terms.
...
After completion, prompt user: "Translation saved. To further review and polish, reply **精细润色** or **refine**."
...
**Archiving Routine**:
1. **Metadata Prep**: Ensure the final translation has YAML frontmatter with `title: Saved - [Title]`, `tags: [Archive, Translation]`, `date: YYYY-MM-DD`, and `url: [Source URL]`.
2. **Move to Vault**: Rename and move the final `translation.md` to `03-Resources/Saved - [Original Title].md`.
...
Source: {source-path}
Languages: {from} -> {to}
Archive Path: 03-Resources/Saved - {Title}.md
